  ...
  onStart: async function ({ api, event, args }) {
    ...
    if (opponentID === "bot" && player_white === "bot") {
      ...
      try {
        ...
        // تحديث الـ FEN في DB والدور للمستخدم
        // نجد اللعبة اللتي أنشأناها للتو
        for (const [id, g] of activeGames) {
          if (g.threadID === threadID && g.player_white === "bot" && g.status === "active") {
            await updateGame(id, { fen: startFen, current_turn: senderID });
            break;
          }
        }
        if (global.db) {
          ...
          if (col) {
            await col.updateOne(
              { threadID, player_white: "bot", status: "active" },
              { $set: { fen: startFen, current_turn: senderID } }
            );
          }
        }
      } catch (e) {
